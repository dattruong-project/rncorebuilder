...
const FlatListBuilder = React.forwardRef((props, ref) => {
    const { activityIndicatorColor = 'black', autoscrollToTopThreshold = 100, data, enableAutoscrollToTop, FooterLoadingIndicator, HeaderLoadingIndicator, ListHeaderComponent, ListFooterComponent, onEndReached = () => Promise.resolve(), onStartReached = () => Promise.resolve(), onRefresh = () => Promise.resolve(), showDefaultLoadingIndicators = true, } = props;
    ...
    const renderHeaderLoadingIndicator = () => {
        ...
        if (!onStartReachedInProgress && !refreshing)
            return null;
    ...
    return (<>
        <FlatList {...props} ref={ref} progressViewOffset={50} onEndReached={maybeCallOnEndReached} onStartReached={maybeCallOnStartReached} onRefresh={handleRefresh} refreshing={refreshing} maintainVisibleContentPosition={{
            autoscrollToTopThreshold: enableAutoscrollToTop
                ? autoscrollToTopThreshold
                : undefined,
            minIndexForVisible: 1
        }} ListHeaderComponent={renderHeaderLoadingIndicator} ListFooterComponent={renderFooterLoadingIndicator}/>
      </>);
});
